...
import com.easy.readbook.repository.SearchBookRepository
import com.easy.readbook.repository.SearchMatchRepository
...
class SearchViewModel : BaseListViewModel() {
     ...
     *  书籍源
     */
    val liveBooks = MutableLiveData<ArrayList<com.easy.readbook.room.entity.Book>>()
    ...
    fun onSearch(key: String) {
        ...
        if (log == null) {
            log = com.easy.readbook.room.entity.LocalSearchKey()
                .apply {
                this.key = key
    ...
    private suspend fun doSearch(readSource: BookSource, key: String) {
        withContext(Dispatchers.IO) {
            SearchBookRepository.doSearch(readSource, key, object : SearchBookRepository.SearchRepositoryCallback {
                override fun onSearchBook(book: ArrayList<com.easy.readbook.room.entity.Book>?, isSuccess: Boolean) {
