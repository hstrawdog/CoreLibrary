 ...
 *
 * @Author : huangqiqiang
 * @Package : com.rongji.shangwenwan.weight.flowlayout
...
public class TagFlowLayout extends FlowLayout
    ...
    /**
     * 最小选中 必须有一个
     * 11 13 添加 配个 adapter select 使用
     */
    private boolean mMinSelect = false;
    ...
    public TagFlowLayout(Context context, AttributeSet attrs, int defStyle) {
        ...
        mSelectMax = ta.getInt(R.styleable.TagFlowLayout_max_select, -1);
        ta.recycle();
    }
        ...
        for (int i = 0; i < adapter.getCount(); i++) {
            ...
            tagViewContainer.setOnClickListener(new OnClickListener() {
                ...
                public void onClick(View v) {
                    if(mTagAdapter.checkSelect(position)){
                        return;
                    }
    ...
    private void doSelect(TagView child, int position) {
        if (!child.isChecked()) {
            //处理max_select=1的情况
            if (mSelectMax == 1 && mSelectedView.size() == 1) {
                ...
                if (mSelectMax > 0 && mSelectedView.size() >= mSelectMax) {
                    return;
                }
                setChildChecked(position, child);
                mSelectedView.add(position);
            ...
            // 只选中了一个  return
            if (mMinSelect && mSelectedView.size() == 1) {
                return;
            }
