...
object SharedPreferencesUtil {
    ...
    @Synchronized
    fun putObject(context: Context,fileName :String, key: String?, `object`: Any) {
        //PreferenceManager.getDefaultSharedPreferences(context);
        val sp = context.applicationContext.getSharedPreferences(fileName, Context.MODE_PRIVATE or Context.MODE_MULTI_PROCESS)
        val editor = sp.edit()
    ...
    @Synchronized
    fun getObject(context: Context,fileName :String, key: String?, defaultObject: Any?): Any? {
        return try {
            //  PreferenceManager.getDefaultSharedPreferences(context); // 会出现空指针=
            val sp = context.applicationContext.getSharedPreferences(fileName, Context.MODE_PRIVATE or Context.MODE_MULTI_PROCESS)
            if (defaultObject is Boolean) {
